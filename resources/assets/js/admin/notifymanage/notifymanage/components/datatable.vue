    ...
    export default {
        ...
        methods: {
            ...
            confirm_delete(){
                var vm = this
                vm.deleting = true
                axios.delete(vm.configUrl.API_ADMIN_NOTIFY_MANAGE_DELETE_LIST_NOTIFY,
                    {
                        params: {
                            list_id_notify : vm.id_item_selected
                        }
                    }).then(data => {
                    vm.notifys = vm.notifys.filter(value =>{
            ...
            getNotifies(){
                var vm = this
                axios.get(vm.configUrl.API_ADMIN_NOTIFY_MANAGE_NOTIFIES).then(data => {
            ...
            confirm_delete_item(id){
                var vm = this
                vm.deleting = true
                axios.delete(vm.configUrl.API_ADMIN_NOTIFY_MANAGE_DELETE_NOTIFY(id)).then(data => {
                    vm.notifys = vm.notifys.filter(value =>{
                        return value.id !=id
                    })
                    var rows_selected = vm.fnGetSelected(vm.table)
